...
import { loginSchema, registerSchema, sanitizeInput } from "../middleware/inputValidation";
import ejs from "ejs";
...
function render(view: string, data: any = {}) {
  try {
    const viewPath = join(process.cwd(), 'views', view)
...
export const authRoutes = new Elysia()
  ...
  .get("/login", () => new Response(render("login.ejs"), { headers: { "Content-Type": "text/html; charset=utf-8" } }))

  .post("/login", async ({ sanitizedBody, set, request }) => {
  ...
  .get("/register", () => new Response(render("register.ejs"), { headers: { "Content-Type": "text/html; charset=utf-8" } }))
